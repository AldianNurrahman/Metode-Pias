...
if (metodeSelect) metodeSelect.value = metodeParam;
...
if (subjudul) subjudul.innerText = titles[metodeParam];
...
function hitung() {
  ...
  if (!fxInput || isNaN(a) || isNaN(b) || isNaN(n) || n <= 0) {
    document.getElementById('output').innerHTML = `<p class="error">⚠️ Input tidak valid!</p>`;
    return;
  }
  ...
  try {
    ...
    document.getElementById('output').innerHTML = `<p class="error">❌ Fungsi tidak valid!</p>`;
    return;
  }
